...
public class Snake {
    ...
    //moveDir是run方法的一个周期中，通过键盘读取的，蛇头想要改变方向
    /**
     * 这段的逻辑是，我们先从Board的键盘监听处读取玩家想要改变的蛇的方向，但是我们不直接把蛇的方向设置成获取的方向
     * 因为如果玩家的run方法的一个周期中多次按下不同的方向键，可能会导致一些BUG，我们先记录“玩家想要改变成”的方向
     * 然后再移动的时候，获取这个想要改变的方向（moveDir）与现在的方向（snakeDir）进行判断再处理。
     */
    private Food food;
...
//        reset();
    ...
    /**
     * 创建一格蛇身体（SnakePos类）对象并随机设置行列，将其返回
    ...
    public void snakeMove()
    {
        ...
        //创建Direction枚举类的四个引用
        if ((moveDir != null)&&!((snakeDir == up && moveDir == down)||(snakeDir == down && moveDir == up)||(snakeDir == left && moveDir == right)||(snakeDir == right && moveDir == left)))
            snakeDir = moveDir;
        ...
        //根据最新蛇头方向，确定新的蛇头在哪个快生成，修改新蛇头的行列坐标
